...
| 设备身份 | Manufacturer=`aidaegis`；设备名=`Air Conditioner`；序列号随机生成并以 MAC 后四位结尾 |
...
### Broadlink RM Mini 3 LED PCB（V1.2）接线

若使用拆下的 `RM mini 3 LED PCB` 红外板：

| 板丝印 | 线色（常见） | 接到 ESP32-S3 |
|--------|--------------|---------------|
| `+` | 红 | **3V3**（优先；5V 时接收头 OUT 可能超过 S3 耐压） |
| `-` | 黑 | **GND**（与开发板共地） |
| 接收头旁 OUT / `IR` | 黄 | **GPIO4**（`BOARD_IR_RX_GPIO`） |
| `LED`（驱动红外发射） | 绿 | **GPIO5**（`BOARD_IR_TX_GPIO`） |

- 学习时对准黑色接收头（U2）按空调遥控任意键；发射管朝向空调。
- 开机/开始学习日志应出现 `IR RX GPIO4 idle level=1`；若为 `0`，先查供电与黄线是否接错到 LED。
- 学习约 60 s 无信号会超时并恢复按钮；学习中可再点 **Cancel** 取消。
...
| Manufacturer / 设备名 | `main/CHIPProjectConfig.h`、`CMakeLists.txt` | `aidaegis` / `Air Conditioner` |
...
2. 设备未入网时，屏上显示 Matter 二维码与数字配对码；内容由固件当前 `MT:...` **动态生成**，与串口 onboarding 打印一致。屏上会提示 **请用2.4G WiFi**。手机 BLE 连上后屏会改为显示「配对中...」；若配网失败/取消，会先提示 **请重新配网 / 请用2.4G WiFi**，再回到配对码。配网成功后恢复完整界面。
...
| 设备名（ProductName / NodeLabel） | `Air Conditioner` | 手机里显示的名称；用户可在 App 中改名 |
| SerialNumber | `RRRRRRRRMMMM` | 首次启动生成：8 位随机十六进制 + Wi‑Fi STA MAC 后 4 位；写入 `chip-factory` NVS，并在 Root Basic Information（及桥接模式的 Bridged Device Basic Information）上 **创建可选 SerialNumber 属性** 后，控制器才会显示；仅写 NVS 时 Apple Home 序列号会为空 |
...
| 触摸左右/上下与显示不一致 | LCD MX 与触控已解耦；勿再随 `BOARD_LCD_MIRROR_X` 翻转触控。若仍反了，改 `board_pins.h` 里 `BOARD_TOUCH_MIRROR_X` / `BOARD_TOUCH_MIRROR_Y` |
| 屏不亮 / 花屏 | 查 SPI 脚 12/13/14/21/47、背光 48、供电与 `board_pins.h`；串口应有 `backlight on` / `UI ready`。若只有黑屏无 UI，查 `UI init failed` / LVGL OOM |
